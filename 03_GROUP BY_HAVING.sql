...
SELECT DECODE(SUBSTR(EMP_NO, 8,1), '1', '남', '2', '여' ) 성별, 
	...
	SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8,1), '1', '남', '2', '여' ) -- 별칭 사용 X(별칭 해서 X)
ORDER BY "인원 수"; -- 별칭 사용 가능 O (SELECT절 해석 완료)
...
--> WHERE절은 각 컬럼 값에 대한 조건(SELECT 문 해석 문서를 잘 기억하는 것이 중요 !!)
/*
5 : SELECT
1 : FROM
2 : WHERE
...
-- TO_NUMBER(SUBSTR(TO_CHAR(HEIRE_DATE), 1, 4)) >=2000
GROUP BY JOB_CODE 
ORDER BY 1;;
...
--
-- 모두
-- EMPLOYEE 체이블에서 부서별로 같은 직급인 사원의 수를 조회
-- (부서코드 오름차순, 직급코드 내림차순)
...
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 --> 한사람의 급여가 300백만 이상이라는 조건(요구사항 충족 X)
GROUP BY DEPT_CODE 
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE 그룹 중 급여 펴윤이 3백만 이상인 그룹만 조회
ORDER BY DEPT_CODE;
...
FROM EMPLOYEE
GROUP BY JOB_CODE
HAVING COUNT(*) <= 5 -- AHVING절에는 그룹 함수가 반드시 작성된다!
ORDER BY 1;
...
-- ROLLYP : GROUP BY 절에서 가장 먼저 작성된 컬럼의 중간 집계를 처리하는 함수 
...
-- (주의사항!) 집합 연ㅅ나자를 사용하기 위한 SELECT문들은
