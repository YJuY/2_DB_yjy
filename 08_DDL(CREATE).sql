...
-- DML(Data Manipulation Langu	age) : 데이터 조작 언어, 
--									   테이블의 데이터를 삽입, 수정, 삭제하는 언어
-- TCL(Transction Control Language) : 트랜잭션 제어 언어
...
/* 자료형
    ...
    CHAR(크기) : 고정길이 문자형 (2000BYTE) ##학번,전화번호 
        -> ex) CHAR(10) 컬럼에 'ABC' 3BYTE 문자열만 저장해도 10BYTE 저장공간을 모두 사용. 
        
    VARCHAR2(크기) : 가변길이 문자형 (4000 BYTE) #주소
        -> ex) VARCHAR2(10) 컬럼에 'ABC' 3BYTE 문자열만 저장하면 나머지 7BYTE를 반환함.
        
    DATE : 날짜 타입
    BLOB : 대용량 이진 데이터 (4GB) #동영상,이미지,코드 (지양하는방식?속도적문제때문에)
    CLOB : 대용량 문자 데이터 (4GB)
...
CREATE TABLE "MEMBER"(
	...
	MAMBER_NAME VARCHAR(30),
	MAMBER_SSN CHAR(14),  -- 991213-1234567
...
COMMENT ON COLUMN "MEMBER".MAMBER_SSN IS '회원 주민 등록 번호';
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER" VALUES('MEM02', 'QWER1234', '김영희', '971005-1122334', SYSDATE);

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO "MEMBER" VALUES('MEM03', '987POIU', '박철수', '940524-2233445', DEFAULT);

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
--INSERT INTO 테이블명(컬럼명1, 컬럼명2, 컬럼명3)
--VALUES(값1, 값2, 값3);

INSERT INTO "MEMBER"(MEMBER_ID, MEMBER_PWD,MAMBER_NAME)
VALUES('MEM04', '456ASDF', '이지연');
...
CREATE TABLE USER_USED_PK(
    ...
    USER_ID VARCHAR2(20) CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼 레벨
    					-- 제약조건명 지정
    USER_PWD VARCHAR2(30) NOT NULL,
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
    ...
    EMAIL VARCHAR2(50)
    -- 테이블 레벨
--    , PRIMARY KEY USER_NO_PK PRIMARY KEY(USER_NO)
...
CREATE TABLE USER_USED_FK(
  ...
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE/*(GRADE_CODE)*/ -- 컬럼 레벨
  											-- 컬럼명 미작성 시 USER_GRACE 테이블의 PK를 자동 참조
  -- 테이블 레벨
-- , FOREING KEY(GRADE_CODE) FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRAKE
 							--> FROEIGN KEY라는 단어는 테이블 레벨에서만
);
...
--ORA-02292: 무결성 제약조건(KH_YJY.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
CREATE TABLE USER_USED_CHECK(
  ...
  USER_PWD VARCHAR2(30) NOT NULL,
  USER_NAME VARCHAR2(30),
  GENDER VARCHAR2(10) CONSTRAINT GANDER_CHECK CHECK( GENDER IN('남','여')),
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50)
);
...
--  FK는 부모 테이블에 행르 추가, 수정, 삭제 하는게 간편함 
...
CREATE TABLE USER_TEST(
	...
	USER_ID VARCHAR2(20) CONSTRATINT UK_USER_ID UNIQUE,
	USER_PWD VARCHAR2(20) CONSTRATINT NN_USER_PWD NOT NULL,
