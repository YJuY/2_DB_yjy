/* VIEW
	...
	***** VIEW 사용 시 주의 사항 ******
	1) 가상의 테이브(실제 테이블 X) -> ALTER 구문 사용 불가
	2) VIEW를 이용한 DML(INSERT/UPDATE/DELETE)가
	   가능한 경우도 있자만
	   많은 제약이 따르기 때문에 SELECT 용도로만 사용 하는 것을 권장.
...
-- INSERT 시 NULL이 삽입됨. ('DO', NULL, 'L3')

-- 만약 VIEW에 포함되지 않은 컬럼(DEPT_TITLE)에 
-- NOT NULL 제약조건이 설정되어 있는 경우
-- 정상적인 INSERT가 진행되지 않는다!
--> ORA-01400: NULL을 ("KH_YJY"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다
