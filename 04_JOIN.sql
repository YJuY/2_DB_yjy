...
-- (참고) JOIN은 서로 다른 테이블의 행을 하났기 이어붙이기 때문에
-- 		 시간이 오래 걸리는 단점이 있다!
/* 
...
-- 컬럼 수를 기준으로 JOIN
--> 왼편에 작성된 테이블의 모든 컬럼이 결과에 포함되어야 한다(JOIN이 안돼는 행도 결과 포함)
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE 
...
--SELECT EMP_NAME 사원명E, BONUS 보너스포인트E, DEPT_TITLE 부서명D, LOCAL_NAME 근무지역명L
--FROM EMPLOYEE
--JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
--JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
...
--SELECT EMP_NAME 사원명E, JOB_NAME 직급명J, DEPT_TITLE 부서명D, LOCAL_NAME 근무지역명L
--FROM EMPLOYEE
--JOIN JOB USING(JOB_CODE)
--JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
--JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
...
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME 사원명E, JOB_NAME 직급명J, SALARY 급여E, SAL_LEVEL
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN SAL_GRADE USING (SAL_LEVEL)

--WHERE TO_NUMBER(SAL_LEVEL)  > MIN_SAL;

--SALARY*(1+NUL(BONUS,0))*12 엱봉ㅇ
/* 5 : SELECT
 * 1 : FROM (+ JOIN)
 * 2 : WHERE
 * 3 : GROUP BY
...
SELECT EMP_NAME 사원명E, LOCAL_NAME 근무지역명L, NATIONAL_NAME 국가명L
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN NATIONAL ON(LOCATION_ID = NATIONAL_CODE)
WHERE NATIONAL_NAME ='KO';
...
-- 단, JOIN, IN 사용할 것
SELET EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE
NATURL JOIN JOB
NATURAL JOIN JOB
CHERE BOUNS IS NULL
AND JOB-CODE IN ('J4', 'J7');
